...
char **splitIntoYamlStrings(char *string, int *lengthOut) {
    ...
    for (int i = 0; i <= strlen(string); ++i) {
        ...
        if (character == '\n' || character == '\0') {
            ...
            array[(*lengthOut) - 1] =
                malloc(sizeof(char) * (strlen(tmp_string) + 1));
            ...
            tmp_string =
                realloc(tmp_string, sizeof(char) * (tmp_string_length + 1));
            ++tmp_string_length;
        }
    }
...
void findValues(Hashmap *hashmap, char *prevKey, int currentIndex,
                int prevIndex, char **array, int arrayLength) {
    int lastFlag = 0;
    ...
    for (int i = currentIndex; i < arrayLength - 1; ++i) {
        ...
        if (nextLevel == currentLevel &&
            currentLevel == (countTabs(array[currentIndex + 1]))) {
            ...
            if (isArrayElement(currentKey)) {
                ...
                currentKey =
                    malloc(sizeof(char) * (countDigits(arrayElementIndex)));
        ...
        if (nextLevel == currentLevel + 1) {
            ...
            findValues(hashmap, newKey, i + 1, currentIndex, array,
                       arrayLength);
            lastFlag = 1;
        }
    }
